...
class PromptManager {
    constructor(app, pluginDir) {
        ...
        // 使用 adapter.basePath 获取 Vault 的物理路径，确保可以被 fs 读取
        // 注意：manifest.dir 是相对路径，如 ".obsidian/plugins/smart-input-pro"
        this.basePath = this.app.vault.adapter.basePath;
        this.defaultsPath = path.join(this.basePath, pluginDir, 'prompts', 'defaults.js');
        this.userPath = path.join(this.basePath, pluginDir, 'prompts', 'user.js');
    ...
    load() {
        try {
            // Load defaults
            const defaults = this.loadModule(this.defaultsPath);
            if (defaults) {
                this.defaults = defaults;
            } else {
                console.error('[SmartInputPro] Defaults prompt file not found or invalid:', this.defaultsPath);
                // 尝试回退到相对路径读取（以防 basePath 获取失败）
                this.defaults = { pipeline: {}, modules: {} };
            }

            // Load user overrides
            if (fs.existsSync(this.userPath)) {
                const user = this.loadModule(this.userPath);
                if (user) {
                    this.user = user;
                } else {
                    console.error('[SmartInputPro] Failed to parse user prompts. Keeping existing file to prevent data loss.');
                    new Notice('SmartInputPro: 用户提示词文件(user.js)加载失败，请检查语法错误。已临时使用默认配置。', 10000);
                    // Do NOT call saveUser() here, protecting the user's edits
                }
            } else {
...
class SmartInputProPlugin extends Plugin {
    async onload() {
        Logger.info('Loading Smart Input Pro plugin');
        ...
        if (this.settings.pipeline?.stage1_classification_prompt) {
            this.promptManager.migrateFromSettings(this.settings);
    ...
    getPersistedSettings(settingsOverride = null) {
        ...
        out.voiceEnabled = !!s.voiceEnabled;
    ...
    async navigateAfterSave(filePath, options = 'end') {
        try {
            ...
            console.warn('navigateAfterSave failed:', err);
        }
    }
    ...
    async unifiedCallAI(params) {
        ...
        const protocol = protocolMap[provider] || 'openai'; // 默认 fallback 到 openai
...
class SmartInput extends Modal {
    ...
    onOpen() {
        ...
        // 为根容器添加类，便于定制关闭按钮与顶部间距
        this.modalEl.addClass('smart-input-modal-root');
        contentEl.addClass('smart-input-modal');
        
        // 标题紧贴上沿，减少上方空白
        const title = contentEl.createEl('h2', { 
            text: '💬 智能录入',
            cls: 'smart-input-title',
            attr: { style: 'font-size: 1.4em; margin-top: 10px; margin-bottom: 18px; color: #512E5F;' }
        });
        ...
        // 倒计时提示文字：移至输入框外部（位于输入框下方的左侧）避免与输入内容重叠
        this.countdownEl = contentEl.createEl('div', {
            cls: 'smart-input-countdown-hint',
            text: '输入内容后将开始自动录入倒计时......'
        });
        ...
        // 按钮容器（左侧放“我先想想”，右侧放“确认”）
        const buttonContainer = contentEl.createEl('div', {
            cls: 'smart-input-button-container',
            attr: { style: 'display: flex; align-items: center; gap: 8px; margin-top: 8px;' }
        });

        // 新增：拒绝自动提交按钮（好听的名字：我先想想）
        const pauseButton = buttonContainer.createEl('button', {
            text: '我先想想',
            cls: 'sip-btn-pause',
            attr: { title: '切换为手动提交模式：不会自动提交，需要点击“确认”提交' }
        });
        pauseButton.onclick = () => {
            this.manualMode = !this.manualMode;
            // 切换按钮文案
            pauseButton.textContent = this.manualMode ? '恢复自动提交' : '我先想想';
            pauseButton.setAttribute('title', this.manualMode ? '已启用手动提交模式，点击可恢复自动提交' : '切换为手动提交模式：不会自动提交，需要点击“确认”提交');
            // 停止任何倒计时
            ...
            // 优化：点击“我先想想”后自动重新聚焦到输入框，并将光标置于文本末尾
            // 使用 setTimeout 保证在按钮状态与计时器更新后再进行聚焦，避免与渲染冲突
            if (this.textArea && typeof this.textArea.focus === 'function') {
                setTimeout(() => {
                    try {
                        ...
                        console.warn('[SmartInputPro] 重新聚焦输入框失败：', e);
                    }
                }, 0);
            }
        };
        ...
        // 底部哲理文字
        const quoteContainer = contentEl.createEl('div', {
            cls: 'smart-input-quote-container',
            attr: { style: 'margin-top: 12px; margin-bottom: 6px;text-align: center;' }
        });

        const quoteText = quoteContainer.createEl('p', {
            cls: 'smart-input-quote-text',
            attr: { style: 'font-size: 0.9em; color: var(--text-muted); font-style: italic;' }
        });
        ...
        const quotes = [
            ...
            "用你的双脚丈量大地，用你的双眼看遍世界。",
            "旅程总有一天会迎来终点，不必匆忙。",
            "凡事讲究时机，创造时机，再抓住时机。这才是人生的秘诀。",
            "阴阳有序，生死无常。嘛，看见什么都别大惊小怪的。",
            "一杯沙砾，也能映出世界。一朵刹那，也能见到永恒。",
            "真正的智慧，是连接世间万物，是让每个人都找到属于自己的答案。"
        ];
    ...
    updateCountdownDisplay() {
        if (this.countdownEl) {
            if (this.manualMode) {
                this.countdownEl.textContent = '已切换为手动提交模式：请点击右侧“确认”提交';
            } else if (this.hasStartedCountdown) {
                this.countdownEl.textContent = `${this.countdownSeconds}s后将自动录入......`;
            } else {
                this.countdownEl.textContent = '输入内容后将开始自动录入倒计时......';
            }
        }
    }
            ...
            try {
                ...
                try {
                    ...
                    if (assetTriggers.includes(item.subcategory)) {
                        this.helperLogAsset(this, baseVariables, optimizedText).catch(err => {
                            try {
...
SmartInputProPlugin.prototype.processCategoryFoodWishlist = async function(optimizedText) {
    try {
        ...
        if (parsed && parsed.shop_name) {
            // 构造待尝清单条目（Markdown 格式）
            let wishlistItem = `- [ ] ${parsed.shop_name}`;
            if (parsed.specialty) wishlistItem += ` ${parsed.specialty}`;
            if (parsed.location) wishlistItem += ` ${parsed.location}`;
            
            // 追加到待尝清单（统一入口：settings -> resolver -> fallback）
            const wishlistFile = this.resolveFoodWishlistPath();
            await this.appendToFile(wishlistFile, wishlistItem);
            
            // 自动打开并定位到新添加的内容（统一跳转模块，支持按文本定位）
            await this.navigateAfterSave(wishlistFile, wishlistItem);
            ...
            return { success: true, message: `美食信息已添加到待尝清单` };
        } else {
            throw new Error('Failed to extract food information');
        }
    } catch (error) {
        // 记录降级处理的错误日志
        await this.logError(error, {
        ...
        try { new Notice(`美食清单处理失败（错误原因：${error.message}），降级归入INBOX`); } catch (_) {}
...
SmartInputProPlugin.prototype.switchToReadingView = async function() {
    try {
        ...
        console.warn('[SmartInputPro] 切换阅读视图失败:', e);
    }
};
...
class SmartInputProSettingTab extends PluginSettingTab {
    ...
    display() {
        ...
        containerEl.empty();
        const ensureStyle = () => {
            const id = 'sip-ide-textarea-style';
            ...
            style.textContent = `
                ...
                .sip-sidebar-item:hover {
                    background: var(--background-secondary-alt);
                    color: var(--text-normal);
                }
                .sip-sidebar-item.is-active {
                    background: var(--background-primary);
                    color: var(--text-normal);
                    border: 1px solid var(--background-modifier-border);
                }
                .sip-sidebar-item > span {
                    overflow: hidden;
                ...
                .sip-prompt-textarea::-webkit-scrollbar,
                .sip-prompt-textarea::-webkit-scrollbar-thumb,
                .sip-prompt-textarea::-webkit-scrollbar-corner {
                    cursor: default;
                }
                .sip-prompt-textarea.sip-prompt-textarea--compact {
                    font-size: 11px;
                    line-height: 1.4;
                }
                .sip-prompt-textarea:focus {
                    outline: none;
                }
                .sip-var-hints {
                    display: flex;
                ...
                .sip-prompt-editor__titleText {
                    font-weight: 700;
                    color: var(--text-normal);
                }
                .sip-prompt-editor__subtitle {
                    margin-top: 2px;
                    color: var(--text-muted);
                    font-size: 12px;
                }
                .sip-prompt-editor__textarea {
                    width: 100%;
                    min-height: 360px;
                    resize: vertical;
                }

                .sip-accordion details {
                    border: 1px solid var(--background-modifier-border);
                    border-radius: 12px;
                    background: var(--background-primary);
                    margin-bottom: 12px;
                ...
                .sip-accordion summary:hover {
                    background: var(--background-secondary-alt);
                }
                .sip-accordion summary::-webkit-details-marker {
                    display: none;
                }
                ...
                .sip-accordion details[open] summary:after {
                    transform: rotate(90deg);
                }
                .sip-accordion details[open] {
                    border-color: var(--interactive-accent);
                }
                .sip-accordion details > div {
                    padding: 12px;
        ...
        containerEl.createEl('h3', { text: 'Smart Input Pro 设置' });
        containerEl.createEl('p', { text: '所有配置通过此面板写入 data.json，并作为插件的唯一运行入口。' });

        const tabs = [
            { key: 'strategy', label: '🤖 策略与资源' },
            { key: 'pipeline', label: '🌐 全局管线' },
            { key: 'modules', label: '🗂️ 业务模块' },
            { key: 'advanced', label: '⚙️ 高级' }
        ];

        if (!this._activeTab) this._activeTab = 'strategy';
        if (!tabs.some(t => t.key === this._activeTab)) this._activeTab = 'strategy';

        const tabBar = containerEl.createDiv({ cls: 'sip-tabs' });
        tabs.forEach((t) => {
            const btn = tabBar.createEl('button', { cls: `sip-tab${this._activeTab === t.key ? ' is-active' : ''}`, text: t.label });
        ...
        const renderStrategy = () => {
            ...
            const primaryCard = contentEl.createDiv({ cls: 'sip-card' });
            const pTitle = primaryCard.createDiv({ cls: 'sip-card-title' });
            const pTitleLeft = pTitle.createDiv();
            pTitleLeft.createDiv({ cls: 'sip-card-titleText', text: '主力大模型' });
            pTitleLeft.createDiv({ cls: 'sip-card-subtitle', text: '服务商 / 模型 / 账号' });

            new Setting(primaryCard)
                .setName('服务商')
                .addDropdown((drop) => {
                    providerIds.forEach((pid) => drop.addOption(pid, pid));
                    drop.setValue(primary.provider || providerIds[0] || '');
                    drop.onChange(async (value) => {
                        primary.provider = value;
                        const models = resources?.[value]?.models || [];
                        primary.model = models.includes(primary.model) ? primary.model : (models[0] || '');
                        primary.accountLabel = ensureAccountLabel(value, primary.accountLabel || 'Default');
                        await save();
                        this.display();
                    });
                });
            ...
            new Setting(primaryCard)
                .setName('模型')
                .addDropdown((drop) => {
                    const pid = primary.provider || providerIds[0] || '';
                    const models = resources?.[pid]?.models || [];
                    models.forEach((m) => drop.addOption(m, m));
                    drop.setValue(primary.model || models[0] || '');
                    drop.onChange(async (value) => {
                        primary.model = value;
                        await save();
                    });
            ...
            const primaryAccounts = resources?.[primary.provider]?.accounts || [];
            if (primaryAccounts.length <= 1) {
                new Setting(primaryCard)
                    .setName('账号')
                    .addText((t) => {
                        t.setValue(primaryAccounts[0]?.label || 'Default').setDisabled(true);
                    });
                primary.accountLabel = primaryAccounts[0]?.label || primary.accountLabel || 'Default';
            } else {
                new Setting(primaryCard)
                    .setName('账号')
                    .addDropdown((drop) => {
                        primaryAccounts.forEach((a) => {
                            const label = String(a?.label || '').trim();
                            if (label) drop.addOption(label, label);
                        });
                        drop.setValue(ensureAccountLabel(primary.provider, primary.accountLabel || 'Default'));
                        drop.onChange(async (value) => {
                            primary.accountLabel = value;
                            await save();
                        });
                    });
            }

            const backupCard = contentEl.createDiv({ cls: 'sip-card' });
            const bTitle = backupCard.createDiv({ cls: 'sip-card-title' });
            const bTitleLeft = bTitle.createDiv();
            bTitleLeft.createDiv({ cls: 'sip-card-titleText', text: '备用大模型' });
            bTitleLeft.createDiv({ cls: 'sip-card-subtitle', text: '是否启用 / 服务商 / 模型 / 账号' });

            new Setting(backupCard)
                .setName('是否启用')
                .addToggle((toggle) => {
                    toggle.setValue(backup.enabled !== false).onChange(async (value) => {
            ...
            new Setting(backupCard)
                ...
                .addDropdown((drop) => {
                    providerIds.forEach((pid) => drop.addOption(pid, pid));
                    drop.setValue(backup.provider || providerIds[0] || '');
                    drop.setDisabled(backup.enabled === false);
                    drop.onChange(async (value) => {
                        backup.provider = value;
                        const models = resources?.[value]?.models || [];
                        backup.model = models.includes(backup.model) ? backup.model : (models[0] || '');
                        backup.accountLabel = ensureAccountLabel(value, backup.accountLabel || 'Default');
                        await save();
                        this.display();
                    });
                });
            ...
            new Setting(backupCard)
                .setName('模型')
                .addDropdown((drop) => {
                    const pid = backup.provider || providerIds[0] || '';
                    const models = resources?.[pid]?.models || [];
                    models.forEach((m) => drop.addOption(m, m));
                    drop.setValue(backup.model || models[0] || '');
                    drop.setDisabled(backup.enabled === false);
                    drop.onChange(async (value) => {
                        backup.model = value;
                        await save();
            ...
            const backupAccounts = resources?.[backup.provider]?.accounts || [];
            if (backupAccounts.length <= 1) {
                new Setting(backupCard)
                    .setName('账号')
                    .addText((t) => {
                        t.setValue(backupAccounts[0]?.label || 'Default').setDisabled(true);
                    });
                backup.accountLabel = backupAccounts[0]?.label || backup.accountLabel || 'Default';
            } else {
                new Setting(backupCard)
                    .setName('账号')
                    .addDropdown((drop) => {
                        backupAccounts.forEach((a) => {
                            const label = String(a?.label || '').trim();
                            if (label) drop.addOption(label, label);
                        });
                        drop.setValue(ensureAccountLabel(backup.provider, backup.accountLabel || 'Default'));
                        drop.setDisabled(backup.enabled === false);
                        drop.onChange(async (value) => {
                            backup.accountLabel = value;
                            await save();
                        });
                    });
            }

            const quotaCard = contentEl.createDiv({ cls: 'sip-card' });
            const qTitle = quotaCard.createDiv({ cls: 'sip-card-title' });
            const qTitleLeft = qTitle.createDiv();
            qTitleLeft.createDiv({ cls: 'sip-card-titleText', text: '当日屏蔽日期' });
            qTitleLeft.createDiv({ cls: 'sip-card-subtitle', text: '用于主力额度耗尽时当日自动切换到备用' });

            new Setting(quotaCard)
                .setName('日期')
                .addText((t) => {
                    t.setValue(String(s.strategy.quotaBlockDate || '')).setDisabled(true);
                })
                .addButton((btn) => {
                    btn.setButtonText('清除').onClick(async () => {
                        s.strategy.quotaBlockDate = '';
                        await save();
                        this.display();
                    });
                });

            const resCard = contentEl.createDiv({ cls: 'sip-card' });
            const resTitle = resCard.createDiv({ cls: 'sip-card-title' });
            const resTitleLeft = resTitle.createDiv();
            resTitleLeft.createDiv({ cls: 'sip-card-titleText', text: '资源池' });
            resTitleLeft.createDiv({ cls: 'sip-card-subtitle', text: '服务商、Base URL、模型列表与多账号密钥' });

            const resWrapper = resCard.createDiv({ cls: 'sip-accordion' });
            providerIds.forEach((pid) => {
                const r = resources[pid] || (resources[pid] = { name: pid, baseUrl: '', models: [], accounts: [{ label: 'Default', apiKey: '', apiKeyPath: '' }] });
                if (!Array.isArray(r.models)) r.models = [];
                if (!Array.isArray(r.accounts) || !r.accounts.length) r.accounts = [{ label: 'Default', apiKey: '', apiKeyPath: '' }];

                const details = resWrapper.createEl('details');
                const summary = details.createEl('summary');
                summary.createSpan({ text: `${pid} (${r.name || ''})` });
                const body = details.createDiv();

                new Setting(body)
                    .setName('展示名称')
                    .addText((t) => {
                        t.setValue(String(r.name || '')).onChange(async (value) => {
                            r.name = value.trim();
                            await save();
                            summary.textContent = `${pid} (${r.name || ''})`;
                        });
                    });
                ...
                new Setting(body)
                    .setName('Base URL')
                    .addText((t) => {
                        t.setValue(String(r.baseUrl || '')).onChange(async (value) => {
                            r.baseUrl = value.trim();
                            await save();
                        });
                    });
                ...
                addPromptEditor(
                    ...
                    (v) => {
                        ...
                        if (primary.provider === pid && !models.includes(primary.model)) primary.model = models[0] || '';
                        if (backup.provider === pid && !models.includes(backup.model)) backup.model = models[0] || '';
                    },
                    [],
                    { minHeight: '140px' }
                );

                const accountsWrap = body.createDiv();
                r.accounts.forEach((acc, idx) => {
                    if (!acc) return;
                    if (!acc.label) acc.label = `Account ${idx + 1}`;

                    new Setting(accountsWrap)
                        .setName(`账号名称 #${idx + 1}`)
                        .addText((t) => {
                            t.setValue(String(acc.label || '')).onChange(async (value) => {
                                const prev = String(acc.label || '').trim();
                                acc.label = value.trim() || prev || `Account ${idx + 1}`;
                                if (primary.provider === pid && primary.accountLabel === prev) primary.accountLabel = acc.label;
                                if (backup.provider === pid && backup.accountLabel === prev) backup.accountLabel = acc.label;
                                await save();
                                this.display();
                            });
                        })
                        .addExtraButton((b) => {
                            b.setIcon('trash');
                            b.onClick(async () => {
                                const removed = r.accounts.splice(idx, 1);
                                const removedLabel = removed?.[0]?.label;
                                if (!r.accounts.length) r.accounts = [{ label: 'Default', apiKey: '', apiKeyPath: '' }];
                                if (primary.provider === pid && removedLabel && primary.accountLabel === removedLabel) primary.accountLabel = ensureAccountLabel(pid, 'Default');
                                if (backup.provider === pid && removedLabel && backup.accountLabel === removedLabel) backup.accountLabel = ensureAccountLabel(pid, 'Default');
                                await save();
                                this.display();
                            });
                        });
                    ...
                    new Setting(accountsWrap)
                        .setName('API Key')
                        .setDesc('优先使用明文；为空则读取路径')
                        .addText((t) => {
                            t.setValue(String(acc.apiKey || '')).onChange(async (value) => {
                    ...
                    new Setting(accountsWrap)
                        .setName('API Key 路径')
                        .addText((t) => {
                            t.setValue(String(acc.apiKeyPath || '')).onChange(async (value) => {
                                acc.apiKeyPath = value.trim();
                                await save();
                            });
                        });
                });
                ...
                new Setting(body)
                    .setName('新增账号')
                    .addButton((btn) => {
                        btn.setButtonText('新增').onClick(async () => {
                            const base = 'Account';
                            const existing = new Set((r.accounts || []).map(a => String(a?.label || '').trim()).filter(Boolean));
                            let n = 1;
                            let label = `${base} ${n}`;
                            while (existing.has(label)) { n++; label = `${base} ${n}`; }
                            r.accounts.push({ label, apiKey: '', apiKeyPath: '' });
                            await save();
                            this.display();
                        });
                    });
        ...
        const renderModules = () => {
            ...
            const layout = contentEl.createDiv({ cls: 'sip-layout-row' });
            const sidebar = layout.createDiv({ cls: 'sip-sidebar' });
            const detail = layout.createDiv();

            moduleIds.forEach((mid) => {
                const m = modules[mid];
                const item = sidebar.createDiv({ cls: `sip-sidebar-item${this._activeModuleId === mid ? ' is-active' : ''}` });
                const label = this.plugin.getCategoryDisplayName ? this.plugin.getCategoryDisplayName(mid) : mid;
                item.createSpan({ text: label });
                item.setAttr('title', mid);
                item.addEventListener('click', () => {
                    this._activeModuleId = mid;
                    this.display();
                });
            });
            ...
            const title = card.createDiv({ cls: 'sip-card-title' });
            const titleLeft = title.createDiv();
            ...
            const moduleTitle = titleLeft.createEl('div', { cls: 'sip-card-titleText', text: String(s.ui.moduleDisplayName?.[mid] || moduleTitleFallback) });
            titleLeft.createDiv({ cls: 'sip-card-subtitle', text: mid });
            attachInlineEdit(moduleTitle, () => String(s.ui.moduleDisplayName?.[mid] || moduleTitleFallback), async (v) => {
                await setModuleDisplayName(mid, v);
                this.display();
            });

            const stEnabled = new Setting(card).setName(getFieldLabel('module.enabled', '启用模块'));
            attachInlineEdit(stEnabled.settingEl.querySelector('.setting-item-name'), () => getFieldLabel('module.enabled', '启用模块'), async (v) => {
                await setFieldLabel('module.enabled', v, '启用模块');
                this.display();
            });
            stEnabled.addToggle((toggle) => {
                toggle.setValue(m.enabled !== false).onChange(async (value) => {
                    m.enabled = value;
                    await save();
                });
            });

            const stOpt = new Setting(card).setName(getFieldLabel('module.enableOptimization', '启用语义优化'));
            attachInlineEdit(stOpt.settingEl.querySelector('.setting-item-name'), () => getFieldLabel('module.enableOptimization', '启用语义优化'), async (v) => {
                await setFieldLabel('module.enableOptimization', v, '启用语义优化');
                this.display();
            });
            stOpt.addToggle((toggle) => {
                toggle.setValue(!!m.enableOptimization).onChange(async (value) => {
                    m.enableOptimization = value;
                    await save();
                });
            });

            if ('targetPath' in m) {
                const stPath = new Setting(card).setName(getFieldLabel('module.targetPath', '文件保存路径'));
                attachInlineEdit(stPath.settingEl.querySelector('.setting-item-name'), () => getFieldLabel('module.targetPath', '文件保存路径'), async (v) => {
                    await setFieldLabel('module.targetPath', v, '文件保存路径');
                    this.display();
                });
                stPath.addText((t) => {
                    t.setValue(String(m.targetPath || '')).onChange(async (value) => {
                        m.targetPath = value.trim();
            ...
            if ('templatePath' in m) {
                const stTpl = new Setting(card).setName(getFieldLabel('module.templatePath', '模板路径'));
                attachInlineEdit(stTpl.settingEl.querySelector('.setting-item-name'), () => getFieldLabel('module.templatePath', '模板路径'), async (v) => {
                    await setFieldLabel('module.templatePath', v, '模板路径');
                    this.display();
                });
                stTpl.addText((t) => {
                    t.setValue(String(m.templatePath || '')).onChange(async (value) => {
                        m.templatePath = value.trim();
            ...
            if ('extractionPrompt' in m || mid) {
                ...
                addPromptEditor(
                    ...
                    { titleKey: 'module.extractionPrompt', titleFallback: '核心提取提示词' }
                );
            }
            ...
            if (mid === 'bill') {
                addPromptEditor(
                    ...
                    ['${text}'],
                    { minHeight: '240px', titleKey: 'bill.asset_log_prompt', titleFallback: '资产信息提取（可选）' }
                );
            }
        };

        const renderAdvanced = () => {
            const card = contentEl.createDiv({ cls: 'sip-card' });
            const title = card.createDiv({ cls: 'sip-card-title' });
            const titleLeft = title.createDiv();
            titleLeft.createDiv({ cls: 'sip-card-titleText', text: '功能开关' });
            titleLeft.createDiv({ cls: 'sip-card-subtitle', text: '影响插件入口与日志记录行为' });
            ...
            new Setting(card)
                .setName('桌面端语音录入')
                .setDesc('是否启用桌面端语音录入入口')
                .addToggle((toggle) => {
                    toggle.setValue(!!s.voiceEnabled).onChange(async (value) => {
                        s.voiceEnabled = !!value;
                        await save();
                    });
                });
            ...
            if (s.enableTokenCostTracking) {
                new Setting(card)
                    .setName('Token 记录文件路径')
                    .addText((t) => {
                        t.setValue(String(s.tokenCostFilePath || '')).onChange(async (value) => {
                            s.tokenCostFilePath = value.trim();
                            await save();
                        });
                    });
            }
        ...
        if (this._activeTab === 'strategy') renderStrategy();
        if (this._activeTab === 'pipeline') renderPipeline();
        if (this._activeTab === 'modules') renderModules();
        if (this._activeTab === 'advanced') renderAdvanced();
    }
}
